...
if (instance_exists(obj_paladin)) {
    ...
    if (jugador.x < x) {
        image_xscale = -0.4; // Mira a la izquierda con escala fija
    } else {
        image_xscale = 0.4;  // Mira a la derecha con escala fija
    }

    image_yscale = 0.4; // Escala vertical fija
...
switch (estado) {
    case "idle":
        sprite_index = spr_kael_idle;
        image_speed = 0.2;
        break;

    case "caminar":
        sprite_index = spr_kael_walk;
        image_speed = 1;
        break;

    case "atacar":
        sprite_index = spr_kael_atk;
        image_speed = 1;
        break;

    case "morir":
        sprite_index = spr_kael_idle; // Cambiar a spr_kael_morir si lo tenés
        image_speed = 1;
        break;
}

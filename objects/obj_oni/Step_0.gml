...
if (instance_exists(obj_paladin)) {
  ...
  if (jugador.x < x) {
    image_xscale = 0.8; // Mira a la izquierda
} else {
    image_xscale = -0.8;  // Mira a la derecha
}

    image_yscale = 0.8; // Escala vertical fija
...
switch (estado) {
    case "idle":
        sprite_index = Spr_oni_idle;
        image_speed = 1;
        break;

    case "caminar":
        sprite_index = Spr_oni_walk;
        image_speed = 1;
        break;

    case "atacar1":
        sprite_index = Spr_oni_hit1;
        image_speed = 1;
        break;
		
	case "atacar2":
        sprite_index = Spr_oni_hit2;
        image_speed = 1;
        break;

    case "morir":
        sprite_index = Spr_oni_idle; // Cambiar a spr_kael_morir si lo tenés
        image_speed = 1;
        break;
}
